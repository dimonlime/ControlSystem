...
async def insert_vendor_cheque(message: Message, state: FSMContext):
    try:
        ...
        if cheque_date <= datetime.now():
            ...
            await message.answer(f'Введите дату не позднее чем {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
    except ValueError:
...
async def order_info(callback: CallbackQuery, state: FSMContext):
    ...
    if order.fact_S is not None:
        await callback.message.answer(
            ...
            f'*Фактическое кол-во товара размера S:* {str(order.fact_S)}\n'
            f'*Фактическое кол-во товара размера M:* {str(order.fact_M)}\n'
            f'*Фактическое кол-во товара размера L:* {str(order.fact_L)}\n', parse_mode="Markdown")
    else:
        await callback.message.answer('Нет данных')
...
async def insert_date_cheque(callback: CallbackQuery, state: FSMContext):
    ...
    media_list = []
    if cheque.payment_image is None and order.fish is None:
        cheque_date_datetime = datetime.strptime(cheque.cheque_date, "%Y-%m-%d %H:%M:%S")
        ...
        f'*Цена:* {str(cheque.price)}\n'
        f'*Название магазина:* {str(cheque.vendor_name)}\n'
        f'*Статус чека:* {str(cheque.cheque_status)}, *Кол-во дней до оплаты:* {days_left.days}\n', parse_mode="Markdown"))
        media_list.append(InputMediaPhoto(media=cheque.cheque_image_id))
        await callback.message.answer_media_group(media=media_list)
...
async def check_all_orders(message: Message, state: FSMContext):
    await state.clear()
    if message.from_user.id in senders:
        await message.answer('Выберите заказ:', reply_markup=await kb.inline_all_orders_send())
